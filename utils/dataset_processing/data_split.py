...
def get_test_dataset(dataset):
    ...
    for split in dataset:
        for wd in range(0, len(dataset[split])):
            # print(dataset['train'][wd])
            sentence = dataset[split][wd]['translation']['sn'].strip()
            for wdd in sentence.split(' '):
                ...
                if difflen < 2 and difflen > -3 and len(slp1expected) >= len(slp1word1) and len(slp1expected) >= len(slp1word2) and len(fullslp1expected) <= maxcompoundlen and len(fullslp1expected) >= inwordlen:
                    ...
                    if slp1word1[:start] == slp1expected[:start] and slp1word2[newlen:] == slp1expected[end:]:
                        ...
                        datalist.append([slp1word1, slp1word2, slp1expected, fullslp1expected, start, end, fullslp1word1, fullslp1word2, wd, 1, split])
                    ...
                    datalist.append([slp1word1, slp1word2, slp1expected, fullslp1expected, max(start, 1), end, fullslp1word1, fullslp1word2, wd, 0, split])
